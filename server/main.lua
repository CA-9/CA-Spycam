...
RegisterNetEvent('spycams:server:placed', function(uuid, coords, rotation, onFloor)
    ...
    if Player.Functions.RemoveItem(Config.SpycamItem, 1) then
        ...
        TriggerEvent('CA-Spycam:server:placed', playerId)
        TriggerClientEvent('CA-Spycam:client:placed', -1, playerId)
...
RegisterNetEvent('spycams:server:removed', function(uuid, destroyed)
    ...
    if destroyed then
        if Spycams[playerId] > 0 then
            ...
            TriggerEvent('CA-Spycam:server:destroyed', playerId)
            TriggerClientEvent('CA-Spycam:client:destroyed', -1, playerId)
        end
    else
        if Player.Functions.AddItem(Config.SpycamItem, 1) then
            ...
            TriggerEvent('CA-Spycam:server:removed', playerId)
            TriggerClientEvent('CA-Spycam:client:removed', -1, playerId)
        end
    end
