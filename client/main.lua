...
function Camera.Activate()
    ...
    if Spycam.Cam then
        DestroyCam(Spycam.Cam, false)
        Spycam.Cam = nil
    end

    -- Create new camera with proper parameters
    Spycam.Cam = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
    ...
    if dist > Config.SignalDistance or currentCam.destroyed then
        ...
        -- Only apply screen effect if not in normal mode
